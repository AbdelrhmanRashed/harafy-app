...
export default function LocationEditInput({
  ...
  const { mutate: saveLocation, isPending: isSaving } = useUpdateProviderData();
  ...
  return (
    <div className="flex w-full flex-col items-center gap-2">
      ...
      <div className="flex w-full items-center gap-2 border-b pb-2">
        <input
          className="border-primary/30 bg-background text-foreground focus:border-primary h-11 min-w-0 flex-1 rounded-md border px-3 py-1 text-sm outline-none disabled:opacity-60"
      ...
      <div className="flex w-full items-center justify-end gap-2">
        <button
          ...
          onClick={onCancel}
          disabled={isSaving}
          className="border-primary/30 bg-background text-foreground hover:bg-primary/10 cursor-pointer rounded-md border px-3 py-1 text-sm font-semibold outline-none disabled:cursor-not-allowed disabled:opacity-50"
        >
          إلغاء
        </button>
